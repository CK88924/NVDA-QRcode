...
def resize_background_image(background_path, target_size=(512, 512), alpha=180):
    """多步調整背景圖大小，保持長寬比並降低透明度"""
    try:
        ...
        # 調整透明度
        alpha_mask = Image.new("L", target_size, alpha)
        final_background.putalpha(alpha_mask)
...
def create_qrcode_with_image_background(data, background_path, output_dir, qr_size=(200, 200), border_size=20):
    """生成二維碼並疊加到背景圖片"""
    try:
        # 創建QR碼
        qr = qrcode.QRCode(
        ...
        # 增加白色邊框
        bordered_qr_image = Image.new("RGBA", (qr_size[0] + border_size * 2, qr_size[1] + border_size * 2), "white")
        bordered_qr_image.paste(qr_image, (border_size, border_size))
        ...
        qr_width, qr_height = bordered_qr_image.size
        position = ((bg_width - qr_width) // 2, (bg_height - qr_height) // 2)

        # 將二維碼粘貼到背景圖上
        resized_background.paste(bordered_qr_image, position, bordered_qr_image)
...
class QRCodeApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.select_button.setFocusPolicy(Qt.StrongFocus)  # 設置按鈕可用 Tab 鍵選中

        self.generate_button = QPushButton("生成二維碼", self)
        self.generate_button.clicked.connect(self.generate_qrcode)
        self.generate_button.setFocusPolicy(Qt.StrongFocus)  # 設置按鈕可用 Tab 鍵選中
    ...
    def keyPressEvent(self, event):
        """覆蓋全局鍵盤事件，讓 Enter 鍵觸發按鈕點擊"""
        if event.key() in {Qt.Key_Return, Qt.Key_Enter}:
            focus_widget = QApplication.focusWidget()
            if isinstance(focus_widget, QPushButton):  # 如果焦點在按鈕上
                focus_widget.click()
        else:
